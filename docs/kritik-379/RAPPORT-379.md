...
- Worktreet havde ved start 179 ændrede, ikke-committede filer under `outputs/kritik-skole*` fra ældre kørsler. Under ordren (kl. 19:35) blev 23 billeder under `outputs/kritik-374/` også ændret, af noget uden for mine kørsler: `scripts/kritik-379.mjs` skriver kun til `outputs/kritik-379/`. De er ikke mine, jeg har ikke rørt dem, og de er ikke med i mine commits. Træet er derfor ikke rent i den forstand. Mine egne filer er alle committet.
