// ORDRE 387 — sikkerhedslinen (kopi af outputs/377/koer-verify.mjs): `npm run build` + `npm run lint` + alle
...
// plus bundtstørrelser fra dist/assets. Skrives til outputs/387/verify-<fase>.json.
// Kørsel: node outputs/387/koer-verify.mjs foer|efter
...
if (kun) {
  const file = join(ROOT, 'outputs', '387', `verify-${FASE}.json`)
...
writeFileSync(join(ROOT, 'outputs', '387', `verify-${FASE}.json`), JSON.stringify(result, null, 2) + '\n')
