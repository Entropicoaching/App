...
// atlet-visning. Bekræfter statisk at Dashboard.jsx kun er rørt det ene sted
// ordren tillod, og at kø'en først tømmes ind i en NY række, aldrig ryddes
// før en gemning er bekræftet.
...
// --- Dashboard.jsx: præcis ét import + én rendering (grænsen i ordren) ---
const dashboardImports = dashboard.match(/^import AthleteSilentFailNote from '\.\/AthleteSilentFailNote'/gm) || []
assert.equal(dashboardImports.length, 1, 'Dashboard.jsx skal importere AthleteSilentFailNote præcis ét sted')
const dashboardUsages = dashboard.match(/<AthleteSilentFailNote\b/g) || []
assert.equal(dashboardUsages.length, 1, 'Dashboard.jsx skal rendere AthleteSilentFailNote præcis ét sted')
...
console.log('Dashboard.jsx er kun rørt det ene sted ordren tillod (import + rendering af AthleteSilentFailNote).')
